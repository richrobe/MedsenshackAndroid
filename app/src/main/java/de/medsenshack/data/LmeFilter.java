...
public class LmeFilter {
     ...
     * @param a_taps denominator coefficients, can be null. if not null, a[0] must
    ...
    /**
     * Represents a statistical object to keep track of running mean, min and
     * max values.
     *
     * @author sistgrad
     */
    ...
    public static class StdFilter extends LmeFilter {
        public long timeStamp = 0;
        public int maxNum = 0;
        private double meanX = 0;
        private double meanXSquared = 0;
        public double value;
        private MeanFilter meanFilterX;
        private MeanFilter meanFilterXSquared;

        public StdFilter() {
            meanFilterX = new MeanFilter(2);
            meanFilterXSquared = new MeanFilter(2);
        }

        public StdFilter(int maxNum) {
            this.maxNum = maxNum;
            meanFilterX = new MeanFilter(maxNum);
            meanFilterXSquared = new MeanFilter(maxNum);
        }

        public double next(double xnow) {
            meanX = meanFilterX.next(xnow);
            meanXSquared = meanFilterXSquared.next(xnow * xnow);
            value = Math.sqrt(meanXSquared - meanX * meanX);

            //Log.d(TAG, "rr: " + xnow);
            return xnow;
        }

        public String formatStd() {
